(function () {
  ...
  Sphere.prototype.rebound = function(other){
    velThisD = [this.vel[0] - other.vel[0], this.vel[1] - other.vel[1]]
    c1 = (Math.pow(this.radius,3) - Math.pow(other.radius,3)) / (Math.pow(this.radius,3) + Math.pow(other.radius,3));
    c2 = (Math.pow(this.radius,3) * 2) / (Math.pow(this.radius,3) + Math.pow(other.radius,3));
    this.vel = [((velThisD[0]*c1)+other.vel[0])*0.75, ((velThisD[1]*c1)+other.vel[1])*0.75];
    other.vel = [((velThisD[0]*c2)+other.vel[0])*0.75, ((velThisD[1]*c2)+other.vel[1])*0.75];
    debugger;
  }
...
// combine Pos and Vel into one function
  ...
  Sphere.prototype.collide = function() {
    // console.log("" + (this.pos[1]+this.radius >= BounceGame.Game.DIM_Y && this.vel[1] > 0));
    // if (this.collideBall()){
    //   // return true;
    // }
    var result = true;
    if (this.pos[0]-this.radius <= 0 ){
      ...
      result =  true;
    } else if (this.pos[0]+this.radius >= BounceGame.Game.DIM_X){
      this.vel = [- this.vel[0]*this.cor, this.vel[1]]
      if (this.vel[0] < 1 && this.vel[0] > -1){
        this.vel[0] = 0;
    ...
    if (this.pos[1]-this.radius <= 0  && this.vel[1] < 0){
      ...
      result = true;
    } else if (this.pos[1]+this.radius >= BounceGame.Game.DIM_Y && this.vel[1] > 0){
      this.vel = [this.vel[0], -this.vel[1]*this.cor]
